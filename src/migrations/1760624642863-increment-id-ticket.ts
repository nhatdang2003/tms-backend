...
export class IncrementIdTicket1760624642863 implements MigrationInterface {
    name = 'IncrementIdTicket1760624642863'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "tickets" ("id" SERIAL NOT NULL, "code" character varying NOT NULL, "title" character varying NOT NULL, "description" text, "customerName" character varying NOT NULL, "customerPhone" character varying, "customerEmail" character varying, "customerAddress" character varying, "customerLat" double precision, "customerLng" double precision, "status" "public"."tickets_status_enum" NOT NULL DEFAULT 'NEW', "scheduledAt" TIMESTAMP, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "assigned_technician_id" integer, "created_by_id" integer, "updated_by_id" integer, CONSTRAINT "PK_343bc942ae261cf7a1377f48fd0" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_c6e20a830c0f8b571abd331b77" ON "tickets" ("code") `);
        await queryRunner.query(`CREATE TABLE "ticket_comment" ("id" SERIAL NOT NULL, "content" text NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "ticket_id" integer, "user_id" integer, CONSTRAINT "PK_375385ad29b177463987f0a14a8" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "ticket_history" ("id" SERIAL NOT NULL, "action" character varying NOT NULL, "data" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "ticket_id" integer, "actor_id" integer, CONSTRAINT "PK_6d0f4bc32cc3581d6a016bca30a" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "ticket_remind" ("id" SERIAL NOT NULL, "remindAt" TIMESTAMP NOT NULL, "note" text, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "ticket_id" integer, "created_by_id" integer, CONSTRAINT "PK_1b8264615bbe5e0905c47592170" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "ticket_photo" ("id" SERIAL NOT NULL, "type" "public"."ticket_photo_type_enum" NOT NULL DEFAULT 'BEFORE', "url" character varying NOT NULL, "mime" character varying, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "ticket_id" integer, "uploaded_by_id" integer, CONSTRAINT "PK_2b02dc77b34706f2f0f37a60cee" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE INDEX "IDX_7f88a8dfdad0f6cbf7c55d0cf5" ON "ticket_photo" ("type") `);
        await queryRunner.query(`CREATE TABLE "feedback_customer" ("id" SERIAL NOT NULL, "comment" text, "channel" character varying, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "ticket_id" integer, CONSTRAINT "PK_a0bec92e744f506470240ac779c" PRIMARY KEY ("id"))`);
